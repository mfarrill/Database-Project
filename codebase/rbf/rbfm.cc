...
RC RecordBasedFileManager::getColumnFromTuple(const void *tuple, const vector<Attribute> recordDescriptor, string attrName, void *&value)
{
    ...
    memcpy(nullIndicator, tuple, offset);
    int i = 0;
    ...
    for (auto _attr = recordDescriptor.begin(); _attr != recordDescriptor.end(); ++_attr)
    {
        ...
        if (!fieldIsNull(nullIndicator, i))
        {
            ...
            case TypeInt:
                if (isTargetValue)
                {
                    value = malloc(_attr->length);
                    if (value == nullptr)
                        return RBFM_MALLOC_FAILED;
                    memcpy(value, (char *)tuple + offset, _attr->length);
                    return SUCCESS;
                }
                offset += INT_SIZE;
                break;
            ...
            {
                int length;
                memcpy(&length, (char *)tuple + offset, INT_SIZE);
                length += INT_SIZE;
                if (isTargetValue)
                {
                    value = malloc(length);
                    if (value == nullptr)
                        return RBFM_MALLOC_FAILED;
                    memcpy(value, (char *)tuple + offset, length);
                    return SUCCESS;
                }
                offset += length;
            }
...
void RecordBasedFileManager::reorganizePage(void *page)
{
    ...
    auto comp = [](IndexedRecordEntry first, IndexedRecordEntry second) { return first.recordEntry.offset > second.recordEntry.offset; };
